...
namespace Unity.Tutorials.Core.Editor
{
    ...
    class TypedCriterionDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            typeFieldPosition.width -= 20;
            EditorGUI.PropertyField(typeFieldPosition, property.FindPropertyRelative(k_TypeField));

            typeFieldPosition.x += typeFieldPosition.width;
            typeFieldPosition.width = 20;
            if (property.serializedObject.ApplyModifiedProperties() || EditorGUI.EndChangeCheck() || GUI.Button(typeFieldPosition, "!"))
            {
                Debug.Log("end change check? "+ property.serializedObject.targetObject.name+" "+property.FindPropertyRelative(k_TypeField).FindPropertyRelative("m_TypeName").stringValue);
        ...
        void OnCriterionTypeChanged(SerializedProperty parentProperty)
        {
            ...
            Debug.Log("criterionType " + criterionType);
            ...
            if (criterionType != null)
            {
                ...
                Debug.Log("attempt to add to " + parentProperty.serializedObject.targetObject.name);
                ...
                criterionProperty.serializedObject.ApplyModifiedProperties();
                criterionProperty.serializedObject.Update();
                //Debug.Log("updated item at " + criterionProperty.propertyPath);
