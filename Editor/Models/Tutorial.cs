...
namespace Unity.Tutorials.Core.Editor
{
    ...
    {
        [NonSerialized] public TutorialContainer Container;
        ...
        [SerializeField]
        [Tooltip("Use this for a scene that doesnt exist at initial project start")]
        string m_OrOpenSceneName = "";//LINDSAY added this, to open a scene that doesnt exist at initial project start
        ...
        public bool TryGoToNextPage(bool forceIt = false)
        {
            if (forceIt == false)
            {
            ...
            if (newIndex != CurrentPageIndex)
            {
                ...
                RoboAnalytics.RecordProgress(this.name, CurrentPage.name);
        ...
        void LoadScene()
        {
            ...
            else if (!string.IsNullOrEmpty(m_OrOpenSceneName))
            {
                try
                {
                    EditorSceneManager.OpenScene("Assets/"+m_OrOpenSceneName + ".unity");
                }
                catch (ArgumentException)
                {
                    try
                    {
                        EditorSceneManager.OpenScene("Assets/Scenes/" + m_OrOpenSceneName + ".unity");
                    }
                    catch (ArgumentException) { }
                }
            }
            
            //lindsay changed: we don't want an empty scene in our tutes, maybe this should be an option?
            //else
